<template>
  <div
    class="group relative cursor-pointer rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 transition-all duration-200 hover:shadow-lg hover:shadow-purple-500/5 hover:-translate-y-0.5 hover:border-purple-200 dark:hover:border-purple-800"
    @click="$emit('click')"
  >
    <!-- Icon + Category -->
    <div class="mb-4 flex items-start justify-between">
      ...
      <span
        v-if="skill.category"
        class="rounded-full border border-[var(--color-border)] bg-[var(--color-muted)] px-2.5 py-0.5 text-xs font-medium text-[var(--color-muted-foreground)]"
      >{{ skill.category }}</span>
    </div>

    <!-- Name + Description -->
    <h3 class="mb-1.5 text-base font-semibold leading-tight">{{ skill.display_name }}</h3>
    <p class="mb-4 line-clamp-2 text-sm leading-relaxed text-[var(--color-muted-foreground)]">
      {{ skill.description || '暂无描述' }}
    </p>

    <!-- Footer: Author + Version + Arrow -->
...
const iconMap: Record<string, string> = {
  compass: '🔮', document: '📄', career: '💼',
  psychology: '🧠', study: '📚', business: '📊',
  code: '💻', chart: '📈', robot: '🤖',
  heart: '❤️', star: '⭐', globe: '🌐',
}

const displayIcon = computed(() => {
  return iconMap[props.skill.icon || ''] || '✨'
})
